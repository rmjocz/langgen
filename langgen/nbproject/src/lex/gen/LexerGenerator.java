...
import lex.LexemeAnalyser;
import lex.LexemeException;
import lex.SyntaxAnalyser;
import lex.SyntaxException;
import lex.tree.AbstractAscSet;
...
public class LexerGenerator {
  ...
  private boolean lexer_link(PrintStream ps, Node[] nds, String[] nms, int i, int j) {
   if (nds[i].links[j].lx_id!=null) {
    ...
    return false; // any breaks
  }

  private boolean lexer_link_df_na(PrintStream ps, Node[] nds, String[] nms, int i, int j) {
    ps.println("          default:");
  ...
  private boolean lexer_link_df_ad(PrintStream ps, Node[] nds, String[] nms, int i, int j) {
    ps.println("          default:");
    if (nds[i].links[j].lx_id == null) {
      if (nds[i].links[j].opt_eol) {
        ps.print("          e++; f = e;");
  ...
  private boolean lexer_link_eof_df(PrintStream ps, Node[] nds, String[] nms, int i, int j) {
    ...
    if (j<nde.links.length-1) {
      ...
      if (nde.links[j].opt_df_na) { // nde.links[j].cset.size()==0 this is assumed to be true here if any cset exist it will be igored
        return lexer_link_df_na(ps, nds, nms, i, j);
      } else if (nde.links[j].aas.length==0) {
        return lexer_link_df_ad(ps, nds, nms, i, j);
      } else {
        boolean no_breaks = lexer_link(ps, nds, nms, i, j);
        if (nde.opt_en_st) {
        ...
        return no_breaks && !nde.opt_en_st;
      }
    }
  }
  ...
  private void lexer_auto(PrintStream ps, Automata aut) {
    ...
    for(int i=0; i<nds.length; i++) {
      ...
      boolean no_breaks = true;
      for(int j=0; j<nds[i].links.length; j++) {
        no_breaks &= lexer_link_eof_df(ps, nds, nms, i, j);
      }
      ps.println("        }");
      if (i<nds.length-1) {
        if (no_breaks) { 
          ps.println(); // if all links returns and throw not need break keyword outside this case
        } else {
          ps.println("        break;\n"); // but if there at least one break in case then must be break keywaord outside the case
        }
      }
    }
    ps.println("      }");
